...
def f1_eval(gt_dict, val_preds, iou_thres=0.5):
    '''
    计算f1值
    '''
    # classidx_truth = [0, 6, 8]
    # classidx_pred = [2, 5, 7]
    classidx_truth = [33, 39, 38]
    classidx_pred = [2, 5, 7]
    # 1.obtain gt: extract all gt objects for this class
    class_recs = {}
    npos = 0
    for img_id in gt_dict:
        R = [obj for obj in gt_dict[img_id] if obj[-1] in classidx_truth]
        bbox = np.array([x[:4] for x in R])
        det = [False] * len(R)
        npos += len(R)
        class_recs[img_id] = {'bbox': bbox, 'det': det}
    ...
    # 2. obtain pred results
    pred = [x for x in val_preds if x[-1] in classidx_pred]
    img_ids = [x[0] for x in pred]
    confidence = np.array([x[-2] for x in pred])
    BB = np.array([[x[1], x[2], x[3], x[4]] for x in pred])
